...
namespace Bhaptics.SDK2
{
    ...
    public class BhapticsLibrary
    {
        ...
        /// </summary>
        /// <param name="eventId">Name of haptic event which you want to play.</param>
        /// <param name="startMillis">[Windows/macOS/Android Only] The playback start position in milliseconds. The event starts from this point, skipping the part before it. e.g. with startMillis = 20, a 10000ms-long event plays from 20ms to the end (the first 20ms are skipped).</param>
        /// <param name="intensity">The haptic intensity is multiplied by this value.</param>
        /// <param name="duration">The haptic duration is multiplied by this value.</param>
        /// <param name="angleX">Rotate haptic counterclockwise around the global Vector3.up. Valid range is: [0.0f - 360.0f]</param>
        /// <param name="offsetY">Move haptic up or down. Valid range is: [-0.5f - 0.5f]</param>
        ...
        /// <para> [TactGlove DK3 only] Vibration frequency in Hz of the DK3 glove's own actuators. This affects only the DK3 glove itself; it does NOT change the operating frequency of any other connected device. 80 is the device default. Has no effect on DK2 gloves or on the universal (non-Windows/macOS) path. </para>
        /// </param>
        ...
        public static int PlayWaveform(PositionType positionType, int[] motorValues, GlovePlayTime[] playTimeValues, GloveShapeValue[] shapeValues, int frequency = 80, int repeatCount = 0)
        {
            ...
            if (enableUniversal)
            {
                _client.PlayWaveform((int)positionType, requestId, WaveformMotors, WaveformPlayTimes, WaveformShapes, frequency, repeatCount);
            }
